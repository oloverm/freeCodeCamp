...
echo "$($PSQL "SELECT count(*) as n FROM games WHERE winner_goals > 2")"
...
echo "$($PSQL "SELECT DISTINCT name FROM (SELECT winner_id as team_id FROM games WHERE year = 2014 AND round = 'Eighth-Final' UNION SELECT opponent_id as team_id FROM games WHERE year = 2014 AND round = 'Eighth-Final') team_ids INNER JOIN teams ON team_ids.team_id = teams.team_id;")"
